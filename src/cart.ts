...
export class Cart {
  ...
  calculateConditions(condition: any, index: number): boolean {
    if (condition.type === "and") {
      return (
        this.calculateConditions(condition.conditions[0], index) &&
  ...
  calculateCondition(condition: any, index: number): boolean {
    if (condition.type == "attribute") {
      ...
      switch (condition.operator) {
        case "gte": {
          if (
            conditionAttributeValue != undefined &&
  ...
  setNewTaxValues(): void {
    ...
    this.products.map((product) => {
      ...
      newTaxValues = {
        ...newTaxValues,
        [newTax]: (newTaxValues[newTax] || 0) + newTaxValue,
      };
    });
      ...
      switch (outcome.type) {
        case "attribute": {
          if (outcome.disabled != undefined && outcome.disabled) {
            this.products[index].setProductAttributes = {
              ...this.products[index].getProductAttributes,
              [outcome.key]: null,
            };
          }
    ...
    switch (productAttributeWithValue.type) {
      case "number": {
        if (!(typeof value === "number")) {
          ...
          targetCartProduct.setProductAttributes = {
            ...targetCartProduct.getProductAttributes,
            [attributeKey]: value,
          };
        }
        break;
      ...
      case "single-select": {
        if (!(typeof value === "string")) {
          ...
          if (
            productAttributeWithValue?.values?.includes(value) ||
            productAttributeWithValue?.values?.includes(value) === undefined
          ) {
            targetCartProduct.setProductAttributes = {
              ...targetCartProduct.getProductAttributes,
              [attributeKey]: value,
    ...
    if(dependantAttributes != undefined){
      dependantAttributes.map((attr: any) => {
        ...
        if (isNaN(newValueCalculated)) {
          throw new Error(
            "Error calculating the script, possibly missing attribute value.",
          );
        }
