...
class Cart {
    ...
    constructor(productsInCart, totalCartPrice, cartTaxValues) {
        this.productsInCart = productsInCart;
        this.totalCartPrice = totalCartPrice;
        this.cartTaxValues = cartTaxValues;
        //a more bit tle private al bi blo okej če bi bil let? loh je let pomoje
        this.catalog = catalog_json_1.default;
    }
    printCatalog() {
        ...
        if (this.catalog.length > 0) {
            this.catalog.map(productCatalog => {
                if (productCatalog.key === productKey) {
                    ...
                    let existingProductInCart = this.productsInCart.find((productCart) => productCart.getProductKey === productKey);
                    ...
                    //add new product to the cart
                        ...
                        let newProduct = new product_1.Product(productKey, productCatalog.title, productCatalog.price, productCatalog.tax, quantity, new Map);
    ...
    removeProduct(cartIndex) {
        ...
        const productToBeRemoved = this.productsInCart[cartIndex];
    ...
    setAttributeValue(cartIndex, attributeKey, value) {
        const producttoAddAttribute = this.productsInCart[cartIndex];
        ...
        let productWithRules = this.catalog.find(element => element.key === producttoAddAttribute.getProductKey);
        //prveri če rules sploh obstajajo
        if (productWithRules === null || productWithRules === void 0 ? void 0 : productWithRules.hasOwnProperty("rules")) {
